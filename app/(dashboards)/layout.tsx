...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-[#923D41] text-[#f7f0f0] flex flex-col md:flex-row md:h-dvh md:overflow-hidden">
      ...
      <nav className={`hidden md:flex md:flex-col md:shrink-0 md:h-dvh md:bg-[#923D41] md:rounded-r-[60px] lg:rounded-r-[80px] md:shadow-xl md:overflow-hidden md:transition-all md:duration-300 ${
        isSidebarExpanded ? 'md:w-64 lg:w-72' : 'md:w-24'
      }`}>
        {/* Logo and Toggle */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          ...
          <button
            ...
            className="shrink-0 p-2 hover:bg-[#4A0A0A] rounded-lg transition-colors text-[#F2DEDE] aria-label="
        ...
        {isSidebarExpanded && (
          <hr className="mx-4 border-white/20 mb-2" />
        )}

        {/* Nav links — scrollable if content overflows */}
        <ul className="flex flex-col gap-1 flex-1 overflow-y-auto px-3 py-2 scrollbar-none">
          {links.map((item) => (
            <li key={item.label}>
              <Link
                href={item.link}
                className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[#F2DEDE] opacity-80 transition-all duration-150 hover:bg-[#4A0A0A] hover:opacity-100 focus:bg-[#4A0A0A] active:bg-[#4A0A0A] whitespace-nowrap"
                title={item.label}
              >
          ...
          <div className="shrink-0 px-4 pb-6 pt-3">
            <div className="flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2.5 text-[#F2DEDE]">
              ...
              <div className="min-w-0 text-sm leading-tight">
                ...
                <p className="truncate text-md capitalize text-[#F2DEDE]/60">{role ?? "—"}</p>
              </div>
            </div>
          </div>
        )}
