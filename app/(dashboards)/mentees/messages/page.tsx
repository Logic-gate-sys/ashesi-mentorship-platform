  ...
  return (
    <div className="flex flex-col items-center gap-2 px-2 py-8 text-center">
      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FDF1F2]">
        <GraduationCap className="h-4 w-4 text-[#6C1221]" />
      </div>
      <p className="text-xs font-medium text-gray-500">No connected mentors yet</p>
      <p className="text-[11px] leading-relaxed text-gray-400">Once a mentor accepts your request you can chat here.</p>
    </div>
  );
}
...
function EmptyConversations() {
  return (
    <div className="flex flex-col items-center gap-2 px-2 py-8 text-center">
      ...
      <p className="text-[11px] leading-relaxed text-gray-400">Select a mentor on the left to start one.</p>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-gray-400">Pick one from the list or start a new chat with a mentor.</p>
      </div>
    </div>
  );
}
...
export default function MenteeMessagesPage() {
  ...
  const { mentors, isLoading: mentorsLoading, error: mentorsError } = useMenteeConnectedMentors();
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] flex-col gap-4 pb-6">
      ...
      <section className="grid min-h-0 flex-1 gap-4 md:grid-cols-[240px_1fr_2fr]">
        ...
        {/* Col 1 — Connected Mentors */}
        <aside className="hidden min-h-0 overflow-y-auto rounded-3xl border border-[#6A0A1D]/10 bg-white p-3 md:flex md:flex-col">
          <h2 className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-gray-400">My Mentors</h2>
          ...
          {mentorsLoading ? (
            <div className="flex flex-col gap-1">
              {[0, 1, 2].map((i) => <SkeletonMentor key={i} />)}
            </div>
          ) : mentors.length === 0 ? (
            <EmptyMentors />
          ) : (
            <div className="flex flex-col gap-1">
              {mentors.map((mentor) => {
                ...
                const secondary = mentor.company || mentor.industry || "Mentor";
                return (
                  <button
                    key={mentor.userId}
                  ...
                  >
                    <div className="relative h-9 w-9 shrink-0 overflow-hidden rounded-full border border-gray-100">
                      <Image src={mentor.avatarUrl || AVATAR_FALLBACK} alt={displayName} fill unoptimized className="object-cover" />
                    </div>
        ...
        <aside className="min-h-0 overflow-y-auto rounded-3xl border border-[#6A0A1D]/10 bg-white p-3">
          ...
          {isLoading && !conversations.length ? (
            <div className="flex flex-col gap-1">
              {[0, 1, 2, 3].map((i) => <SkeletonConversation key={i} />)}
            </div>
