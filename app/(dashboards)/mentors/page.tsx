...
import { useMentorDashboardStats } from "@/app/_components_and_hooks/_hooks/useMentorDashboardStats";
...
export default function MentorHomePage() {
  const { user } = useAuth();
  const {stats, recentUpdates, pendingRequests, activeMentees, scheduleEvents} = useMentorDashboardStats({role: user?.role, id: user?.id});
  ...
  return (
    <div id="mentor-dash-home" className=" text-accent flex flex-col items-start gap-4">
     <section id="welcome-sec" className="w-full flex flex-col items-start justify-center">
        <h1 className="text-3xl font-bold tracking-tight text-accent">const statsIcons: LucideIcon[] = [User, MailIcon, Clock];
